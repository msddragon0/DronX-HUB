...
print("[DRONX] Carregado com sucesso!")        local CF = game.ReplicatedStorage.Controllers:FindFirstChild("CombatController")
        if CF then
            CbFw2 = debug.getupvalues(require(CF))[2]
        end
    end)
end
...
if not CbFw2 then
    pcall(function()
        for _, v in pairs(game.Players.LocalPlayer.PlayerScripts:GetDescendants()) do
            if v.Name == "CombatFramework" then
                CbFw2 = debug.getupvalues(require(v))[2]
                break
            end
        end
    end)
...
local function equiparArma(tipoForcado)
    ...
    -- Se já tem algo na mão do tipo certo, não troca
    local armaAtual = player.Character:FindFirstChildOfClass("Tool")
    if armaAtual and armaAtual.ToolTip == tipo then
        return
    end
